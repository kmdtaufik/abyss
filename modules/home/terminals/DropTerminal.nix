      ...
      ''
        ...
        wait_for_hypr() {
          for _ in $(seq 1 60); do
            if hyprjson monitors | jq -e 'length > 0' >/dev/null 2>&1 \
               && hyprjson activeworkspace | jq -e '.id | numbers' >/dev/null 2>&1; then
              return 0
            fi
            sleep 0.05
          done
        ...
        get_window_geometry() {
          local addr="$1"
          hyprjson clients | jq -r --arg ADDR "$addr" '.[] | select(.address == $ADDR) | "\(.at[0]) \(.at[1]) \(.size[0]) \(.size[1])"'
        }
        ...
        ensure_floating() {
          local addr="$1"
          local is_float
          is_float=$(hyprjson clients | jq -r --arg ADDR "$addr" '.[] | select(.address == $ADDR) | .floating // false')
        ...
        animate_slide_down() {
          ...
          # Animate slide down
          for _ in $(seq 1 "$SLIDE_STEPS"); do
            local current_y=$((start_y + (step_y * _)))
        ...
        wait_for_hypr
        CURRENT_WS=$(hyprjson activeworkspace | jq -r '.id // empty')
        ...
        terminal_in_special() {
          ...
          addr=$(get_terminal_address)
          if [ -n "$addr" ]; then
            hyprjson clients | jq -e --arg ADDR "$addr" --arg WS "$SPECIAL_WS" 'map(select(.address == $ADDR and .workspace.name == $WS)) | any' >/dev/null 2>&1
          else
            return 1
          fi
        }
